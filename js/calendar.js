    ...
    Calendar.prototype = $.extend({}, Widget.prototype, {
        ...
        bindUI: function() {
            ...
            this._$elem.on('tap', '.pre-mon', function() {
                    me.bind_switch_month_button(function(){
                        me._preMon();
                    })
                })
                //点击头部下一月
        ...
        bind_switch_month_button(callback){
            let me = this;
            let beforeNextSlideStatus = false;
            if (me.isSlideUp()){
                me.set("slideAnimateTime", 100);
                me.slideDown();// 原先是收缩 先恢复展开状态
                beforeNextSlideStatus = true;
            }
            setTimeout(function(){ 
                callback();
            },150);
            if (beforeNextSlideStatus){
                setTimeout(function(){
                    me.set("slideAnimateTime", 200);
                    me.slideUp();
                }, 200)
            }
        },
        ...
        _goPreOrNextMon: function(type, date) {
            ...
            this.updateDateWrapOriginHeight();
